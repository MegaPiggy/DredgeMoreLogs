...
### Creating your mod
1. [Generate your repository from this template](https://github.com/new?template_name=DredgeMoreLogs&template_owner=DREDGE-Mods)
2. Clone your new repository to your machine
3. Open all files in a text editor and do a find replace on `DredgeMoreLogs` to change it to the name of your mod without spaces. For example, `CosmicHorrorFishingBuddies`.
4. Rename all files with `DredgeMoreLogs` in the name to instead be the name of your mod without spaces.
5. Edit `DredgeMoreLogs/mod_meta.json`
6. Edit `DredgeMoreLogs/DredgeMoreLogs.csproj.user`
7. Open `DredgeMoreLogs.sln` in Visual studio
8. Start writing your code in `DredgeMoreLogs.cs`
...
```
{
	...
	"ModGUID": "xen.DredgeMoreLogs",
	"Version": "0.0.1",
	"ModAssembly": "DredgeMoreLogs.dll",
	"MinWinchVersion": "0.2.3",
	"Entrypoint": "DredgeMoreLogs.Loader/Initialize"
}
```
...
- `ModAssembly`: The main `.dll` file for your mod. This should be changed along with all other references to `DredgeMoreLogs` in a find/replace on all files.
...
- In the `OutputPath` line, change `xen.DredgeMoreLogs` to match the `ModGUID` you wrote in `mod_meta.json`.
